...
<head>
    ...
    <style>
        ...
        #log { margin-top: 15px; font-size: 12px; color: #333; background: #eee; padding: 10px; border-radius: 5px; text-align: left; height: 120px; overflow-y: auto; }
    </style>
</head>
<body>
...
<div class="kartu">
    <h2>🚀 RESI PRO</h2>
    <p style="font-size: 12px;">Solusi Print Thermal Modal HP</p>
    ...
    <div id="log">Status: Siap memproses...</div>
</div>
    ...
    async function mulaiProses() {
        ...
        try {
            ...
            for (const file of input.files) {
                log.innerHTML += "<br>> Membaca: " + file.name;
                const bytes = await file.arrayBuffer();
                const pdfLama = await PDFDocument.load(bytes);
                
                // CARA BARU: Pakai copyPages, bukan embed
                const halamanDicopy = await pdfBaru.copyPages(pdfLama, [0]);
                const halAsli = halamanDicopy[0];
                
                // Tambahkan halaman baru
                const halBaru = pdfBaru.addPage([lebarT, tinggiT]);
                
                // Ambil ukuran asli
                const { width, height } = halAsli.getSize();
                const skala = lebarT / width;

                // Gambar halaman asli ke halaman baru
                halBaru.drawPage(halAsli, {
                    x: 0,
                    y: tinggiT - (height * skala),
                    width: width * skala,
                    height: height * skala
                });
            }
            ...
            link.href = url;
            link.download = "RESI_HASIL.pdf";
            link.click();

            log.innerHTML += "<br><b>BERHASIL! Cek Folder Download.</b>";
        } catch (err) {
